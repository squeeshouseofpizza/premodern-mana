...
for c in d:
    if c["name"] == "Rhystic Cave":
        c["notes"] = ("Five-color fixing that any opponent can switch off for one mana, so it should not be counted as "
            "a reliable colored source in any matchup where the opponent holds mana up. The Gatherer ruling of "
...
for i,(n,cols) in enumerate(tap):
    note = ("The simplest shape in the format: enters tapped, taps for one of two colors, no cost, no limit, no "
            "condition. Worth recording precisely because it is the baseline every more complicated dual should be "
            "compared against. Never a turn-one source of either color."
...
for i,(n,main,pair) in enumerate(sac):
    note = ("The colors of the two abilities do not overlap, which makes this cycle a genuine trap for anything "
...
new.append(card("Archaeological Dig","INV",[],[],False,None,None,None,
    ...
    [],None,
    "Untapped colorless land that can convert itself once into any single colored mana. Unlike the Homelands and Masques filters the colored mode is self-sufficient and nets a full mana, because the price is the land rather than mana. A calculator should count it as one colored source of every color, available once."))
...
new.append(card("Keldon Necropolis","INV",["Legendary"],[],False,None,None,None,
    ...
    None,
    "Colorless source. Legendary. The damage ability costs five mana plus a creature plus the tap, so it is a late-game mana sink and never competes with using this as a land."))
...
new.append(card("Forsaken City","PLS",[],[],False,None,None,None,
    ...
    None,
    "Free untapped any-color mana on the turn it lands, and after that every single use costs a card exiled from hand. The first activation is genuinely free, which is why availability sits on the ability rather than the card and why enters_tapped is false. In a deck that empties its hand this stops untapping entirely and becomes a dead land, so the restriction is not a fixed rate like the depletion lands, it is contingent on a resource the mana base cannot supply."))

new.append(card("Meteor Crater","PLS",[],[],False,None,None,None,
    [ab({"mode":"derived","source":"a color of a permanent you control"},1,1,"{T}",True,
        cond={"type":"controls_colored_permanent"})],
    [],None,
    "Derived like Reflecting Pool but from a different pool of objects, and much weaker for it. It reads colors off permanents you control, and lands are almost always colorless, so with only lands on the battlefield this produces nothing at all. It cannot help you cast your first spell, only your second onwards, and only in a color you already have on board. Note it looks at permanents, not lands, so an artifact or a colored creature enables it."))
...
new.append(card("Terminal Moraine","PLS",[],[],False,None,None,None,
    ...
    "A colorless land that can convert itself into any basic for two mana. Unlike the Mirage fetches it costs mana and the fetched land arrives tapped, so the turn you use it costs you three mana of tempo for a land that does nothing until next turn. Colors resolve against the decklist."))
...
for i,(n,cols) in enumerate(pain):
    note = ("Mechanically identical to the Ice Age painlands, and unlike the Tempest cycle these enter untapped. Free "
            "colorless mode with no damage, free colored mode with 1 damage as part of that ability's effect rather "
            "than as a cost. Real turn-one sources of both colors. Together with the Ice Age five these are the "
            "backbone of two-color mana in the format."
