...
for i,(n,types) in enumerate(fetch):
    note = ("Produces no mana itself, so abilities is empty and produces is null; its colours are a property of the "
...
for i,(n,col) in enumerate(cyc):
    note = ("Same shape as the Urza's Saga cycling lands, with one difference: cycling costs a single mana of this "
            "land's own colour rather than {2}. That makes it cheaper in the deck that wants it and unusable in a deck "
            "that cannot produce that colour, which is a real constraint on a card whose whole point is flexibility. "
...
for i,(n,cost,eff) in enumerate(util):
    note = ("Colourless source with a tribal ability that costs coloured mana this land cannot produce, so it never "
            "pays for its own ability and any turn it is used it is a net drain. Records as a colourless source only; "
            "the colour in the activation cost is a requirement, not production, and is exactly the sort of symbol a "
...
new.append(card("Seaside Haven","ONS",[],[],False,None,None,None,
    ...
    None,
    "See Contested Cliffs. Colourless source; the ability needs white and blue from elsewhere and eats a Bird, not the land."))
...
new.append(card("Starlit Sanctum","ONS",[],[],False,None,None,None,
    ...
    None,
    "See Contested Cliffs. Two separate non-mana abilities in different colours; neither produces mana and both compete with the tap."))

new.append(card("Grand Coliseum","ONS",[],[],True,None,None,None,
    [ab(fixed("C"),1,1,"{T}",True),
     ab(choice(*WUBRG),1,1,"{T}",True)],
    [],None,
    "City of Brass that enters tapped and has a free colourless mode. Two structural differences from City of Brass beyond the tapped entry: the damage here is part of the coloured ability's effect rather than a trigger on becoming tapped, so tapping for colourless costs nothing and taps by opponents' effects cost nothing either. Never a turn-one source of anything."))
