...
SKIP = {"pattern": "skips_next_untap",
        "reason": "Using this ability leaves the land tapped through your next untap step, so the coloured mode is available every other turn."}
...
for i,(n,cols) in enumerate(pain):
    note = ("Enters tapped and still deals damage, which makes this cycle strictly worse than the Ice Age painlands "
            "rather than a variant of them. Same two-ability structure: the colourless mode is free of damage and the "
            "coloured mode is not, and the damage is part of that ability's effect rather than a cost, so it cannot be "
...
for i,(n,cols) in enumerate(slow):
    note = ("Enters untapped, and the colourless mode is completely unrestricted: tap it for {C} every turn forever with "
            "no penalty. Only the coloured mode locks it through your next untap step, which is why availability sits on "
            "that ability rather than on the card. Rate works out the same as the Ice Age depletion cycle, usable for "
            "colour on alternating turns, but the mechanism and the free colourless mode are different and a calculator "
...
new.append(card("Ancient Tomb","TMP",[],[],False,None,None,None,
    ...
    [], None,
    "Two colourless per activation, not one. Amount is the whole point of this card and a classifier that records only colour identity loses it entirely. The 2 damage is part of the ability's effect, not a cost, so it is unavoidable and applies on every activation. Untapped, unconditional, unlimited, so it is real turn-one acceleration and the life total is the only limit."))
...
new.append(card("Wasteland","TMP",[],[],False,None,None,None,
    ...
    None,
    "A colourless source until the moment it is not. The sacrifice is on a non-mana ability, so sacrifices_self is false on the mana ability and true on the other_abilities entry. Behaviourally this is cracked early and proactively, which is what separates it from a card like Barbarian Ring that also sacrifices itself but gets held for a late-game finish. Nothing in the schema encodes that intent; the difference lives in the note and in what the ability actually does."))
...
new.append(card("Reflecting Pool","TMP",[],[],False,None,None,None,
    ...
    [], None,
    "Colours resolve against the board, not the card, so produces gets its own mode rather than a list. Two consequences that matter. Alone on the battlefield it produces nothing at all, because it can only mirror what your lands could produce and it has nothing to mirror; a hand of Reflecting Pools makes zero mana. And it mirrors what a land could actually produce right now, so a Tainted Field with no Swamp out contributes nothing to it. In a deck built on the Odyssey filters or the Homelands castles this is much weaker than it looks, because those lands can only produce colour with mana already available."))
...
new.append(card("Maze of Shadows","TMP",[],[],False,None,None,None,
    ...
    None,
    "Colourless source. The shadow ability costs {T} and competes with the mana ability."))
...
new.append(card("Ghost Town","TMP",[],[],False,None,None,None,
    ...
    None,
    "Colourless source with a free escape hatch from Wasteland and other targeted land removal, usable only on an opponent's turn. Bouncing it is not a mana cost but it does cost your next land drop to replay, and it cannot be used to reset itself on your own turn. Not modelled as an availability restriction because using the mana ability imposes nothing; the bounce is optional and independent."))
...
new.append(card("Stalking Stones","TMP",[],[],False,None,None,None,
    ...
    None,
    "Colourless source. Animation costs six but does not tap it and is permanent, so unlike the Urza's Legacy manlands it is a one-time investment rather than a per-turn cost. Still a land after animating, so it keeps producing mana."))
...
new.append(card("Volrath's Stronghold","STH",["Legendary"],[],False,None,None,None,
    ...
    None,
    "Colourless source. Legendary, so a second copy is dead. The recursion ability costs {T} plus two mana, so a turn spent using it is a turn this land is a net drain rather than a source."))
