...
C.append(card("Sisay's Ring","artifact","{4}",4,False,False,False,
  ...
  notes="Four mana for a rock that taps for two. Net positive from the turn after it lands, and never fixes color. cmc is the number that matters most on these: it cannot contribute anything before turn four on an untouched curve."))
C.append(card("Thran Dynamo","artifact","{4}",4,False,False,False,
  [ab(fixed("C","C","C"),3,3,"{T}",True)],
  notes="Same slot as Sisay's Ring for one more mana of output. Colorless only."))
...
C.append(card("Millikin","creature","{2}",2,False,True,False,
  ...
  notes="Colorless mana that costs a card off the top each time. In a deck that wants a full graveyard the mill is upside; anywhere else it is a real cost and additional_cost records it rather than burying it in text."))
...
for n,col in [("White Mana Battery","W"),("Blue Mana Battery","U"),("Black Mana Battery","B"),
    ...
    C.append(card(n,"artifact","{4}",4,False,False,False,
      ...
      notes=("Reads like a storage artifact and works like a bad mana rock. Removing no counters "
             ...
             "card is really a four-mana rock that taps for one of its color."
...
for n,cols in [("Seashell Cameo",["W","U"]),("Drake-Skull Cameo",["U","B"]),
    ...
    C.append(card(n,"artifact","{3}",3,False,False,False,
      ...
      notes=("Three mana for a rock that taps for one of two colors. Fixes without accelerating, "
             "since it costs three to add one. Its value is entirely color, which makes it the "
...
for n,col in [("Marble Diamond","W"),("Sky Diamond","U"),("Charcoal Diamond","B"),
    ...
    C.append(card(n,"artifact","{2}",2,True,False,False,
      ...
      notes=("Two mana, enters tapped, then taps for one of its color every turn. Genuine "
...
for n,cols in [("Skycloud Egg",["W","U"]),("Darkwater Egg",["U","B"]),
    ...
    C.append(card(n,"artifact","{1}",1,False,False,True,
      ...
      notes=("Net zero mana. You pay {2} and get two back, so this never accelerates anything; it "
             "converts generic mana into two specific colors and replaces itself with a card. "
             "self_sufficient is false, so it can never produce your first colored mana, which "
...
for n,cols in [("Dromar's Attendant",["W","U","B"]),("Crosis's Attendant",["U","B","R"]),
    ...
    C.append(card(n,"creature","{5}",5,False,False,True,
      ...
      notes=("Five mana for a 2/2 that can later be turned into three specific colors for one. "
...
C.append(card("Lotus Petal","artifact","{0}",0,False,False,True,
  ...
  notes="Free to cast and produces any color once, so it is one of the few nonland cards in the pool that genuinely can be your first colored mana on turn one. One shot: using it is the end of it. Net plus one on the turn it is used and minus a card overall."))
C.append(card("Mox Diamond","artifact","{0}",0,False,False,False,
  ...
  notes="Free, untapped, any color, every turn, and the price is a land card discarded as it enters. Same replacement-effect shape as the Alliances lands, so entry_cost carries it: if you cannot or will not discard a land it goes straight to the graveyard. In land-count terms it is a wash, which is why it is played as fixing and tempo rather than as ramp."))
C.append(card("Lion's Eye Diamond","artifact","{0}",0,False,False,True,
  ...
  notes="Three mana of one color for free, and the cost is your entire hand, which is why it only functions in decks built to empty their hand first or to use the mana before the discard matters. Instant speed only. produces uses choice_uniform because all three mana are the same chosen color, not three colors."))

# --- filters and converters -------------------------------------------------
C.append(card("Mana Cylix","artifact","{1}",1,False,False,False,
  [ab(choice(*WUBRG),1,0,"{1}, {T}",False)],
  notes="Pure color conversion at net zero. Never accelerates and can never make your first colored mana. The cheapest fixer in the pool and correspondingly the weakest."))
...
C.append(card("Mana Prism","artifact","{3}",3,False,False,False,
  [ab(fixed("C"),1,1,"{T}",True),
   ab(choice(*WUBRG),1,0,"{1}, {T}",False)],
  notes="Two modes, and only the colorless one is self-sufficient. Structurally identical to the Homelands castles: a colorless source with a conversion button attached."))
C.append(card("Phyrexian Lens","artifact","{3}",3,False,False,False,
  [ab(choice(*WUBRG),1,1,"{T}, Pay 1 life",True)],
  notes="Any color, no mana cost to activate, so self_sufficient is true and the price is a life per use. The artifact equivalent of City of Brass, except the life loss is part of the ability rather than a trigger on tapping."))
...
C.append(card("Astrolabe","artifact","{3}",3,False,False,True,
  ...
  [oa("activated","{1}, {T}, Sacrifice","Draw a card at the beginning of the next turn's upkeep.",True)],
  notes="Two mana of one chosen color for one, so net plus one, plus a delayed card. Not to be confused with the Modern Horizons card of a similar name, which is not in this pool."))

# --- board-dependent --------------------------------------------------------
C.append(card("Fellwar Stone","artifact","{2}",2,False,False,False,
  [ab({"mode":"derived","source":"any color a land an opponent controls could produce"},1,1,"{T}",True,
      cond={"type":"opponent_controls_land_producing_mana"})],
  notes="Colors resolve against the opponent's board, not yours and not the card, so it is the one accelerant in the pool whose usefulness is a matchup question. Against a mono-colored deck it is a one-color rock; against nothing it produces nothing. Note it reads colors, so an opponent playing only colorless-producing lands leaves it dead."))
C.append(card("Star Compass","artifact","{2}",2,True,False,False,
  [ab({"mode":"derived","source":"any color a basic land you control could produce"},1,1,"{T}",True,
...
C.append(card("Sol Grail","artifact","{3}",3,False,False,False,
  [ab(choice(*WUBRG),1,1,"{T}",True)],
  notes="The color is chosen as it enters and then fixed for the rest of the game, so the choice mode here is a one-time decision rather than a per-activation one. Treat it as a rock of whichever color you named."))
C.append(card("Charmed Pendant","artifact","{4}",4,False,False,False,
  [ab({"mode":"derived","source":"the colored mana symbols in the milled card's mana cost"},0,None,"{T}, Mill a card",True,
      addl={"action":"mill","count":1},
      var=V("the number of colored pips on the card milled",
            "Zero if the milled card is a land or has no colored pips."))],
...
C.append(card("Phyrexian Altar","artifact","{3}",3,False,False,False,
  [ab(choice(*WUBRG),1,1,"Sacrifice a creature",True,
      addl={"action":"sacrifice","count":1,"target":"creature"})],
  notes="See Ashnod's Altar. One mana instead of two, but any color, and likewise unlimited within a turn."))
...
C.append(card("Kyren Toy","artifact","{3}",3,False,False,False,
  ...
  notes="Charging costs {1} and the tap to buy one future colorless, so like the mana batteries the storage mode loses tempo. The floor is one, not zero."))
...
C.append(card("Lotus Blossom","artifact","{2}",2,False,False,True,
  [ab(varv("any one color"),0,None,"{T}, Sacrifice",True,sac=True,
...
C.append(card("Thran Turbine","artifact","{1}",1,False,False,False,
  ...
  notes="Two free colorless every upkeep that cannot pay for a single spell. The restriction field exists for this card: counted as a source without it, it would look like the best accelerant in the format. It pays for activated abilities only."))
C.append(card("Diamond Kaleidoscope","artifact","{4}",4,False,False,False,
  ...
  [oa("activated","{3}, {T}","Create a 0/1 colorless Prism artifact creature token.")],
  notes="Two steps and both are expensive: four to cast, then three and a tap per token, and each token converts into exactly one mana of any color. Net deeply negative as mana; it exists for decks that want the tokens themselves."))
...
C.append(card("Lotus Guardian","creature","{7}",7,False,True,False,
  [ab(choice(*WUBRG),1,1,"{T}",True)],
  notes="Seven mana for a flying body that taps for one of any color. By the time this is castable the fixing is decoration."))
